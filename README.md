...
Everything happening within AMPEL is triggered by a _process_.
...
All information regarding processes are registred into the database.
The majority of processes are associated with a specific tier but general processes are possible.

A working AMPEL system will execute multiple processes, posssibly concurently, accross the four AMPEL tiers.
This will result in the ingestion and anaylsis of data and the triggering of automated reactions when given data states are detected.
...
Internally, they are just tags in database documents and ampel configuration files.  
...
Note that within AMPEL, different processes requiring the same computation
