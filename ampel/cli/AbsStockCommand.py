...
class AbsStockCommand(AbsCoreCommand, abstract=True):
	...
	def get_select_args_help() -> Dict[str, str]:
		return {
			...
			'modified-after-ts': 'Modified after unix timestamp',
			'modified-after-str': 'Modified after date-time iso string',
			'modified-after-delta': 'Modified after time delta',
			'modified-after-process': 'Modified after last run of process with name',
			'modified-before-ts': 'Modified before unix timestamp',
			'modified-before-str': 'Modified before date-time iso string',
			'modified-before-delta': 'Modified before time delta',
			'modified-before-process': 'Modified before last run of process with name',
	...
	def add_selection_args(self, builder: ArgParserBuilder) -> None:
		...
		builder.add_x_args('match',
			{'name': 'modified-before-str'}, {'name': 'modified-before-ts', 'type': int},
			{'name': 'modified-before-delta', 'action': LoadJSONAction},
			{'name': 'modified-before-process'}
		)
		builder.add_x_args('match',
			{'name': 'modified-after-str'}, {'name': 'modified-after-ts', 'type': int},
			{'name': 'modified-after-delta', 'action': LoadJSONAction},
			{'name': 'modified-after-process'}
		)
	...
	def build_select_model(self, args: Dict[str, Any]) -> UnitModel:
		conf = {
			...
			"modified": self.get_time_model("modified", args),
