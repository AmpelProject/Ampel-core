...
class T3ProjectingStager(T3BaseStager):
	...
	def multi_bla(self, gen: Generator[AmpelBuffer, None, None]) -> Optional[List[T3Record]]:
		"""
				...
				while (x := list(islice(gen, self.chunk_size))):
					...
					for async_res, generator, t3_unit in zip(async_results, generators, all_units):
						...
						if (t3_unit_result := async_res.get()):
							if (z := self.handle_t3_result(t3_unit, t3_unit_result, generator.stocks)):
								ret.append(z)
	...
	def craft_t3_record(self, t3_unit: AbsT3Unit, res: T3Result) -> T3Record:
		t3_rec = super().craft_t3_record(t3_unit, res)
		if self.save_stock_ids:
			rb = next(filter(lambda rb: t3_unit in rb.units, self.run_blocks))
			if rb.stock_ids:
				t3_rec['stock'] = rb.stock_ids
		return t3_rec
