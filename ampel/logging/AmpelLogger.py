...
from logging import Logger, _logRecordFactory # type: ignore
from typing import Dict, TextIO, Optional, Union, Sequence, Any, ClassVar, Literal
...
class AmpelLogger(Logger):

	loggers: ClassVar[Dict[str, 'AmpelLogger']] = {}
	current_logger: ClassVar[Optional['AmpelLogger']] = None # noqa

	default_stream: ClassVar[TextIO] = sys.stdout
	aggregation_ok: ClassVar[bool] = True
	console_logging_option: ClassVar[Literal["default", "compact", "compact", "headerless"]] = "default"
	...
	def _new_logger(
		...
		console_logging_option: Optional[Literal['default', 'compact', 'compacter', 'headerless']] = None
	) -> 'AmpelLogger':
		"""
		...
		:param console_logging_option: aggregation options for console logging.
		See AmpelLoggingStreamHandler docstring for more details
		"""
		...
		# Perform import here to avoid cyclic import error
		...
		sh = AmpelLoggingStreamHandler(
			...
			options = console_logging_option
		)
	...
	def quieten_console(self) -> None:
		""" Shortcut for set_console_log_level(logging.WARNING) """
	...
	def verbose(self, msg, *args, **kwargs) -> None:
		""" log custom msg with log level VERBOSE """
			...
			if isinstance(exc_info, tuple):
				for el in traceback.format_exception(
					...
					for ell in el.split('\n'):
						if len(ell) > 0:
							addlog(ell)
							if not AmpelLogger.aggregation_ok:
								AmpelLogger.aggregation_ok = True
				addlog(rv.getMessage())
				rv.msg = "-" * len(rv.getMessage())
	...
	@classmethod
	def break_aggregation(cls) -> None:
		AmpelLogger.current_logger = 1 # type: ignore
