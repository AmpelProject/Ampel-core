	...
	def load(self,
					...
					for el in res:
						...
						if "t2_dependency" in (config_dict := el["config"]):
							for t2conf in config_dict["t2_dependency"]:
								# assume config has been resolved if it is a dict already
								if isinstance(t2conf['config'], int):
		...
		if logger and logger.verbose:
			...
			for col_name in (col_set - set(["stock"])):
				s += f", {col_name}: "
				s += str(sum([1 for k in register for el in register[k][col_name]])) # type: ignore[misc, union-attr]
			logger.info(s)
