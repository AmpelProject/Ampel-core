#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : ampel/pipeline/utils/T2DocsShaper.py
...
class T2DocsShaper():
	...
	def __init__(self, channels_config, channel_names_to_load):
		...
		The parameter array_of_scheduled_t2_runnables used in method 'recude' 
		...
		2nd key: t2 runnable paramId (string)
		...
		# Loop through schedulable t2 runnables 
		for t2_runnable_id in T2RunnableIds:

			# Each entry of dd_full_t2Ids_paramIds_chanlist is also a dict (1st dict key: t2 runnable id)
			self.dd_full_t2Ids_paramIds_chanlist[t2_runnable_id] = dict()
			...
			for i, chan_name in enumerate(channel_names_to_load):
				...
				# t2_runnable_id is of type 'enum', we access the flag label with the attribute 'name'
				paramId = channels_config.get_channel_t2_param(chan_name, t2_runnable_id.name)

				# if paramId was not found, it means the current t2_runnable_id 
				...
				if not paramId in self.dd_full_t2Ids_paramIds_chanlist[t2_runnable_id]:
					self.dd_full_t2Ids_paramIds_chanlist[t2_runnable_id][paramId] = ChannelFlags(0)
				...
				self.dd_full_t2Ids_paramIds_chanlist[t2_runnable_id][paramId] |= self.active_chan_flags[i] 

		
	def reduce_unsorted(self, compound_gen, dict_of_scheduled_t2_runnables):
		"""
			dict_of_scheduled_t2_runnables example:
			d[CHANNEL_SN] = SNCOSMO | AGN
				whereby CHANNEL_SN is of type ChannelFlag
				and SNCOSMO | AGN is of type T2RunnableIds
		"""
		array_of_scheduled_t2_runnables = []
		
		for chan_flag in self.active_chan_flags:
			for key in dict_of_scheduled_t2_runnables.keys():
				if key == chan_flag:
					array_of_scheduled_t2_runnables.append(dict_of_scheduled_t2_runnables[key])	

		self.get_struct(compound_gen, array_of_scheduled_t2_runnables)
	

	def get_struct(self, compound_gen, array_of_scheduled_t2_runnables):
		"""
		----------------------------------------------------------------------------------
		...
		The input parameter of alert ingesters is a list ('array_of_scheduled_t2_runnables') 
		of combinations of scheduled T2 runnable Ids (ampel.flags.T2RunnableIds set by T0 filters). 
		The size of array_of_scheduled_t2_runnables corresponds to the number of "active" channel 
		('active' meaning that the T0 AlertProcessor has loaded the channel)
		
		Strictly considered, an example of array_of_scheduled_t2_runnables could be:
		  [<T2RunnableIds.PHOTO_Z|AGN: 12>, <T2RunnableIds.PHOTO_Z|SNCOSMO: 9>, etc...]
		...
		let's say array_of_scheduled_t2_runnables provides the following information:
		...
		INNER LOOP 1 matches the function parameter 'array_of_scheduled_t2_runnables' with 
		...
		for i, single_channel_scheduled_t2s in enumerate(array_of_scheduled_t2_runnables):
			...
			for t2_runnable_id in single_channel_scheduled_t2s.as_list():

				# Create dict instance if necessary	
				if not t2_runnable_id in t2s_eff:
					t2s_eff[t2_runnable_id] = {}

				# loop through all known paramIds for this t2 runnable
				for paramId in self.dd_full_t2Ids_paramIds_chanlist[t2_runnable_id].keys():
				
					# If channel flag of current channel (index i) is registered in dd_full_t2Ids_paramIds_chanlist
					if self.active_chan_flags[i] in self.dd_full_t2Ids_paramIds_chanlist[t2_runnable_id][paramId]:

						# create flag if necessary
						if not paramId in t2s_eff[t2_runnable_id]:
							t2s_eff[t2_runnable_id][paramId] = ChannelFlags(0)

						# append t0 channel value
						t2s_eff[t2_runnable_id][paramId] |= self.active_chan_flags[i]
		...
		for t2_id in t2s_eff.keys():
			...
			# Loop through parameter ids for current t2 runnable 
