#!/usr/bin/env python
# -*- coding: utf-8 -*-
# File              : ampel/pipeline/utils/T2MergeUtil.py
...
class T2MergeUtil():
	...
	The generated structure (get_struct) is optimized: 
	...
	def __init__(self, channels):
		...
		'T2UnitIds': enum flag *class* (not instance) containing the ids of T2 units
		 -> typically generated with: FlagGenerator.get_T2UnitIds_class(t2_collection)

		NOTE: order of 'channels' matters: the parameter 'array_of_scheduled_t2_runnables'
		used in method 'get_struct' must have the same channel order 
		...
		Values are combinations of channel flags (ChannelFlag class generated by FlagGenerator)
		"""
		...
		# Get reference to generated T2UnitIds enum *class* (not instance) from 
		# the first channel instance in the list (could have been any)
		T2UnitIds = channels[0].T2UnitIds

		# Loop through schedulable t2 units
		for t2_id in T2UnitIds:
			...
			for channel in channels:
				...
				if not run_config in self.dd_full_t2Ids_runConfigs_chanlist[t2_id]:
					self.dd_full_t2Ids_runConfigs_chanlist[t2_id][run_config] = channel.get_flag()
				else:
					# Add current t0 channel to dd_full_t2Ids_runConfigs_chanlist
					# For example: dd_full_t2Ids_runConfigs_chanlist[SCM_LC_FIT]["default"] |= CHANNEL_SN
					self.dd_full_t2Ids_runConfigs_chanlist[t2_id][run_config] |= channel.get_flag()
	...
	def get_struct(self, compound_gen, array_of_scheduled_t2_runnables):
		...
		of combinations of scheduled T2 unit Ids (T2UnitIds set by T0 filters). 
		...
		A concrete example of array_of_scheduled_t2_runnables could be:
		  [<T2UnitIds.PHOTO_Z|AGN: 12>, <T2UnitIds.PHOTO_Z|SNCOSMO: 9>, etc...]
		
		where each position in the array corresponds to a specific channel flag
		(info stored in self.channels[i].get_flag()):
		  [CHANNEL_GRB, CHANNEL_SN, etc...]

		let's bring the two info above in a more compact form for the sake of this documentation:
		Say array_of_scheduled_t2_runnables provides the following information:
		...
		self.dd_full_t2Ids_runConfigs_chanlist which could look like this:
		...
		INNER LOOP 2 adds a third parameter to t2s_eff to take into account that an alert 
		loaded by different channels might result in different compounds!
		...
		for i, single_channel_scheduled_t2s in enumerate(array_of_scheduled_t2_runnables):
			...
			for t2_id in single_channel_scheduled_t2s.as_list():
				...
				for run_config in self.dd_full_t2Ids_runConfigs_chanlist[t2_id].keys():
					...
					if channel.get_flag() in self.dd_full_t2Ids_runConfigs_chanlist[t2_id][run_config]:
		...
		for t2_id in t2s_eff.keys():
			...
			for run_config in t2s_eff[t2_id].keys():
				...
				chan_flags = t2s_eff[t2_id][run_config]

				# Add a 3rd level to t2s_eff dict
				t2s_eff[t2_id][run_config] = dict()

				# Set channel values for each compound id. 
				compound_ids = compound_gen.get_compound_ids(chan_flags)

				# Simple case: there is only one compound if for the current association of channels
				if len(compound_ids) == 1:
					t2s_eff[t2_id][run_config][next(iter(compound_ids))] = chan_flags
				else:
					# channels have different compound id.
					# we must compute the intersection (&) between chan_flags from loop 1 and all the channels
					# returned by CompoundGenerator for a given compound_id
					for compound_id in compound_ids:
						t2s_eff[t2_id][run_config][compound_id] = (
							compound_gen.get_channels_for_compoundid(compound_id) & chan_flags
						)
