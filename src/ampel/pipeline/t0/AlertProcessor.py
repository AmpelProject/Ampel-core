...
class AlertProcessor(DBWired):
	...
	def gather_and_send_stats(self, post_run, t0_stats):
		...
		if "graphite" in self.publish_stats:
			...
			gfeeder = GraphiteFeeder(AmpelConfig.get_config('resources.graphite'))
...
def run_alertprocessor():
	...
	processor = AlertProcessor(mongodb_uri=mongo, publish_stats=["jobs", "graphite"], config=AmpelConfig.get_config())
