...
class TFilter(AbsAlertFilter):
	...
	def set_filter_parameters(self, d):
		...
		self.MinDeltaJD			= d['MinDeltaJD'] 		# remove movers with min time distance in days between detections that pass default filter
		self.MaxDeltaJD			= d['MaxDeltaJD']		# option to require we have (at least one epoch with) two detections within a night (or even shorter timescale)
